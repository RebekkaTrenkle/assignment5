...
pkgs <-c( "rtracklayer","TFBSTools","JASPAR2014",
          "BSgenome.Hsapiens.UCSC.hg18","TxDb.Hsapiens.UCSC.hg18.knownGene" )
...
bioc.libs <-c("Biobase", "affy", "annotate", "hgu95av2.db", 
              "hgu95av2cdf", "estrogen", "limma", "genefilter", 
              "vsn", "GEOquery", "annaffy", "GO.db","KEGG.db")
cran.libs <-c("RColorBrewer", "R.utils", "gplots")


source("http://bioconductor.org/biocLite.R") # get BiocInstaller
biocLite() 
# if they are outdated packages, choose the option to update all the packages
...
# remove stuff larger than 2000 kb
FOXA1 <- FOXA1[width(FOXA1) <= 2000]


# Q 3.4
hist(width(FOXA1), xlab="FOXA1 peak size", col="grey")
...
# 4.2
bp <-barplot(c(length(ER), length(FOXA1)), 
             names=c("ER", "FOXA1")) 
